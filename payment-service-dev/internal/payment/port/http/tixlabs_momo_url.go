...
type MomoGetURLResponse struct {
	URL string `json:"url"`
}

func (s Server) TixlabsGetMomoURL(w http.ResponseWriter, r *http.Request) {
	...
	cmd := command.TixlabsGetURL{
		PaymentProvider: domain.PaymentProvider_MOMO,
	...
	render.JSON(w, r, MomoGetURLResponse{URL: url})
}
